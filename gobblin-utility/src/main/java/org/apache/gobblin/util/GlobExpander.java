...
class GlobExpander {
  ...
  public static List<String> expand(String filePattern) throws IOException {
    ...
    List<GlobExpander.StringWithOffset> toExpand = new ArrayList<GlobExpander.StringWithOffset>();
    toExpand.add(new GlobExpander.StringWithOffset(filePattern, 0));
    while (!toExpand.isEmpty()) {
      GlobExpander.StringWithOffset path = toExpand.remove(0);
      List<GlobExpander.StringWithOffset> expanded = expandLeftmost(path);
      if (expanded == null) {
   ...
   * @throws IOException
   */
  private static List<GlobExpander.StringWithOffset> expandLeftmost(GlobExpander.StringWithOffset
    ...
    List<GlobExpander.StringWithOffset> exp = new ArrayList<GlobExpander.StringWithOffset>();
    for (String string : alts) {
      exp.add(new GlobExpander.StringWithOffset(prefix + string + suffix, prefix.length()));
    }
    return exp;
  }
